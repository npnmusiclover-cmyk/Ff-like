...
# IMPORTANT:
# Railway Variables me sirf API KEY add karo
# Variable Name = API_KEY
# Variable Value = tumhara api key
...
async def num(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=20) as client:
            ...
            try:
                ...
                await msg.edit_text(
                    "❌ INVALID API RESPONSE\nCHECK YOUR API KEY OR API SERVER"
                )
                return
    ...
    for i, item in enumerate(results, start=1):
        ...
        text += (
            ...
            f"📞 ALT MOBILE: {item.get('alternative mobile', 'N/A')}\n"
            f"📡 SIM INFO: {item.get('circle/sim', 'N/A')}\n"
            f"🏠 ADDRESS: {item.get('address', 'N/A')}\n"
            f"🪪 ID NUMBER: {item.get('id number', 'N/A')}\n"
